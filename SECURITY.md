...
This repository is the JavaScript/TypeScript SDK workspace — the SDK itself, the CLI, the LangChain and MCP middleware, and the browser consent widget. This is the code that runs inside other people's agents and services, holding wallets and building presentations, so we treat security reports against it seriously.
...
2. **GitHub Security Advisory:** Use the "Report a vulnerability" button under the **Security** tab of the [HelixID repository](https://github.com/helixid/helix-sdk-js/security/advisories). This opens a private advisory visible only to repository maintainers.
...
- **Wallet and key handling** — encrypted wallet storage, passphrase handling, key serialization, private key leakage through logs or errors
- **Presentation building and verification** — VP construction, local verification paths, replay protection
- **Framework middleware** — credential extraction, injection vectors, and policy-enforcement bypass in the LangChain, MCP, and CrewAI adapters
- **The consent widget** — scope rendering that misrepresents what is being approved, DOM injection, `postMessage` origin validation
- **CLI** — command injection, unsafe file writes, secrets written to disk or shell history
- **Supply-chain concerns** — tampered npm artifacts, typosquat risk on `@helixid/*`, release signing integrity
...
- **Subscribe to advisories:** Watch this repository with "Releases and security advisories" enabled, or subscribe via RSS to the [Security Advisories feed](https://github.com/helixid/helix-sdk-js/security/advisories).
...
- [SDK reference](https://docs.helixid.dev/sdk/sdk-js) — the JS/TS SDK on the docs site
...
- **GitHub Security Advisories:** [github.com/helixid/helix-sdk-js/security/advisories/new](https://github.com/helixid/helix-sdk-js/security/advisories/new)
